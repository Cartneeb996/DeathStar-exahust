...
namespace DeathStarExhaustPort
{
    public partial class MainForm : Form
    {
        ...
        {
            Pen detailPen = new Pen(Color.Red, 2);
            Pen arcPen = new Pen(Color.Blue, width/2);
            SolidBrush exBrush = new SolidBrush(Color.Blue);
            
            offScreen.DrawBezier(detailPen, x - width/2, y, x - width*2, y + height / 10, x - width, y + height / 8, x - width / 2 + detailPen.Width, y + height / 2); // left detail
            offScreen.DrawBezier(detailPen, x + width + width / 2, y, x + width + width * 2, y + height / 10, x + width + width, y + height / 8, x + width + width / 2 - detailPen.Width, y + height / 2); // right detail
            offScreen.DrawArc(arcPen, x - width/2, y + height - height / 9, width*2 - 2, height / 5, 315, 270); // bottom cap
            offScreen.FillRectangle(exBrush, x - width / 2, y, width / 2, height - height / 15);// left side
            offScreen.FillRectangle(exBrush, x + width, y, width / 2, height - height / 15); // right side
        }
